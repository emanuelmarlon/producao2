...
import { StockMovement, Lot } from '@prisma/client';

export class InventoryService {
    async createLot(data: Omit<Lot, 'id' | 'createdAt' | 'updatedAt' | 'quantityCurrent'>) {
    ...
    async createMovement(data: Omit<StockMovement, 'id' | 'createdAt'>) {
    ...
    async getMovementsHistory(filters: { startDate?: Date, endDate?: Date, type?: string, productId?: string }) {
        const where: any = {};
    ...
    async getLotExpirationReport(daysThreshold: number = 30) {
    ...
    async createStockAdjustment(data: { productId: string; lotId?: string; quantityAdjustment: number; reason: string; reference?: string }) {
        return prisma.$transaction(async (tx) => {
            // Get current stock
    ...
    async getAdjustmentHistory(filters: { startDate?: Date; endDate?: Date; productId?: string }) {
        const where: any = {
